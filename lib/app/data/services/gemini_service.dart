...
import 'package:infography/app/utils/api_keys.dart';
import '../models/infographic_model.dart';
import '../models/template_model.dart';

class GeminiService {
  static const String systemPromptText =
      '''You are a professional infographic designer and data visualization expert.
...
''';
  ...
  Future<void> _ensureModelInitialized() async {
    ...
    // Get model name from AppStrings (set by RemoteConfig) or fallback to AIConfig
    final modelName = AppStrings.gemini_model.isNotEmpty
        ? AppStrings.gemini_model
        : AIConfig.modelName;
    final modelString = modelName.startsWith('models/')
        ? modelName
        : 'models/$modelName';

    _model = GenerativeModel(
      model: modelString,
      apiKey: "AIzaSyDybpOjC503ExD_xO6-8XjuGmiklu8_TjM",
      ...
      //  systemInstruction: Content.text('''You are a professional infographic designer and data-visualization expert. Respond ONLY with a single valid JSON object: {\"html\":\"...\", \"css\":\"...\"}. HTML must include head+body resources; CSS must be a complete stylesheet. NO external images — use only icon fonts (Font Awesome / Material) and pure CSS graphics. Mobile-first: every content block must be a .section-16-9 container (width:100vw; height:56.25vw) and use only vw/%/relative units (no px/vh). Produce 6–8 independent 16:9 sections: Title, Executive Summary (4 cards), Statistics (4 compact cards), Detailed Analysis (3–4 progress bars), Charts (CSS pie + bar), Key Insights (3 bullets), Conclusion (2–3 takeaways), Additional Data (before/after). Include 15–20 data points total across sections; include comparisons, percentages, trends, brief insights and optional timeline/metrics/table. All text wrapped in descriptive classes (title, subtitle, statistic, chart, data-point, etc.). Visual rules: 3–5 complementary colors, gradients, glassmorphism, shadows, smooth animations, touch-friendly interactions. Technical: include CSS for .section-16-9, data-cards, compact-cards, progress-bar, pie-chart, comparison-layout, timeline, metric-grid, compact-bullets, simple-summary; charts no taller than ~40% of a section. Ensure strict spacing and no overlap; center content; stack vertically; avoid multi-column horizontal layouts on mobile. Templates to support: 4-card layout, compact cards, progress bars, pie chart, compact bullets, simple summary, before/after comparison — keep HTML patterns as provided. Adapt icons/colors to topic (sports/food/tech/country) but never use external images. Final output must look professional and be fully mobile-optimized.''')
    );
  }

  Future<InfographicModel?> generateInfographic(
    String prompt, {
    ColorTheme? theme,
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        print(
          '🔍 DEBUG: Starting infographic generation for prompt: "$prompt" (Attempt $attempt/$maxRetries)',
        );
        print(
          '🔍 DEBUG: Prompt contains cricket keywords: ${_containsCricketKeywords(prompt)}',
        );
        print(
          '🔍 DEBUG: Prompt contains food keywords: ${_containsFoodKeywords(prompt)}',
        );
        print(
          '🔍 DEBUG: Prompt contains country keywords: ${_containsCountryKeywords(prompt)}',
        );
        print(
          '🔍 DEBUG: Prompt contains tech keywords: ${_containsTechKeywords(prompt)}',
        );
        ...
        // Build template blocks if selections were provided
        final themeBlock = theme?.toPromptBlock() ?? '';
        final layoutBlock = layout?.toPromptBlock() ?? '';

        // Extract theme properties for the prompt
        final primaryColor = theme?.primaryColor ?? '#3182ce';
        final fontFamily = theme?.fontFamily ?? 'Inter';

        final content = [
          Content.text(
            '''INSTRUCTION: Generate a professional, high-density INFOGRAPHIC for: "$prompt".

$themeBlock
$layoutBlock

REQUIRED CONTENT:
- 8-10 major sections with unique titles.
- 20+ specific statistics/data points with detailed explanations.
- Topic-relevant Font Awesome icons (fas fa-...) for every section.
- Modern CSS: Gradients, card-shadows, glassmorphism. Use $primaryColor as the theme base.

OUTPUT FORMAT (JSON):
{
  "html": "The full interior HTML content (no <img> tags)",
  "css": "The complete CSS styles (set font-family: '$fontFamily')"
}
DO NOT return null. FILL the values with rich content.''',
          ),
        ];
        ...
        // ── Groq key rotation ───────────────────────────────────────────
        final List<String> _groqKeys = ApiKeys.groqKeys;
        ...
        for (final key in _groqKeys) {
          try {
                ...
                'messages': [
                  {
                    'role': 'system',
                    'content':
                        'You are a professional design agency. You strictly output JSON {"html": "...", "css": "..."}. You always provide comprehensive, data-rich infographics with 8+ visual sections. NO IMAGES.',
                  },
                  {
                    'role': 'user',
                    'content': (content[0].parts.first as TextPart).text,
                  },
                ],
                'temperature': 0.7,
                'max_tokens': 4096,
            ...
            // Catch both 429 (Rate Limit) and 413 (Payload Too Large relative to TPM)
            if (resp.statusCode == 200) {
              httpResponse = resp;
              break;
            } else if (resp.statusCode == 429 || resp.statusCode == 413) {
              print(
                '🔍 DEBUG: Key hit limit (Status ${resp.statusCode}), rotating to next key...',
              );
              httpResponse = resp;
              continue;
            } else {
            ...
            print('🔍 DEBUG: Error with key rotation: $e');
            continue;
          }
        }
        ...
        if (httpResponse == null || httpResponse.statusCode != 200) {
          throw Exception(
            'Groq API Error: ${httpResponse?.statusCode ?? "Unknown"} - ${httpResponse?.body ?? "No response"}',
          );
        }

        final jsonResp = jsonDecode(httpResponse.body);
        String cleanedResponse = jsonResp['choices'][0]['message']['content'];

        // Debug logging for API response
        print('🔍 DEBUG: Groq API response received on attempt $attempt');
        print(
          '🔍 DEBUG: Response length: ${cleanedResponse.length} characters',
        );
        if (cleanedResponse.isNotEmpty) {
          print(
            '🔍 DEBUG: Response preview: ${cleanedResponse.substring(0, cleanedResponse.length > 200 ? 200 : cleanedResponse.length)}...',
          );
        }
        ...
        if (cleanedResponse.isNotEmpty) {
          ...
          try {
            ...
            print(
              '🔍 DEBUG: Cleaned response length: ${cleanedResponse.length}',
            );
            print(
              '🔍 DEBUG: Cleaned response preview: ${cleanedResponse.substring(0, cleanedResponse.length > 300 ? 300 : cleanedResponse.length)}...',
            );

            Map<String, dynamic> jsonData;
            try {
              // The AI might return the JSON wrapped in markdown or just the JSON string
              String jsonString = cleanedResponse.trim();
              if (jsonString.startsWith('```json')) {
                jsonString = jsonString.substring(
                  7,
                  jsonString.length - (jsonString.endsWith('```') ? 3 : 0),
                );
              ...
              cleanedResponse = jsonString.trim();
              jsonData = jsonDecode(cleanedResponse);
            } catch (e) {
              print(
                '🔍 DEBUG: Initial JSON parsing error, trying regex extraction: $e',
              );
              try {
                ...
                // Fallback for potentially escaped JSON
                try {
                  jsonData = jsonDecode(
                    cleanedResponse.replaceAll(r'\"', r'"'),
                  );
                } catch (e2) {
                  print('🔍 DEBUG: All JSON parsing attempts failed: $e2');
                  rethrow;
                }
              }
            }
            ...
            if (jsonData.containsKey('html')) {
              print(
                '🔍 DEBUG: HTML length: ${jsonData['html'].toString().length}',
              );
              print(
                '🔍 DEBUG: HTML preview: ${jsonData['html'].toString().substring(0, jsonData['html'].toString().length > 200 ? 200 : jsonData['html'].toString().length)}...',
              );
            }
            if (jsonData.containsKey('css')) {
              print(
                '🔍 DEBUG: CSS length: ${jsonData['css'].toString().length}',
              );
            }
            ...
            if (jsonData.containsKey('html')) {
              ...
              if (uniqueImages.length != imageSources.length) {
                ...
                print(
                  '🔍 DEBUG: Total images: ${imageSources.length}, Unique images: ${uniqueImages.length}',
                );
                final duplicates = imageSources
                    .where(
                      (img) =>
                          imageSources.indexOf(img) !=
                          imageSources.lastIndexOf(img),
                    )
                    .toSet();
              ...
              final cricketRelevantImages = imageSources
                  .where(
                    (img) => cricketKeywords.any(
                      (keyword) => img.toLowerCase().contains(keyword),
                    ),
                  )
                  .toList();
              print(
                '🔍 DEBUG: Cricket-relevant images: ${cricketRelevantImages.length}/${imageSources.length}',
              );
              if (cricketRelevantImages.isNotEmpty) {
                print(
                  '🔍 DEBUG: Cricket-relevant image URLs: $cricketRelevantImages',
                );
              }

              // Check image sources diversity
              final unsplashImages = imageSources
                  .where((img) => img.contains('unsplash.com'))
                  .toList();
              final pexelsImages = imageSources
                  .where((img) => img.contains('pexels.com'))
                  .toList();
              final pixabayImages = imageSources
                  .where((img) => img.contains('pixabay.com'))
                  .toList();
              final wikimediaImages = imageSources
                  .where((img) => img.contains('wikimedia.org'))
                  .toList();
              final googleImages = imageSources
                  .where(
                    (img) =>
                        img.contains('google.com') ||
                        img.contains('googleusercontent.com'),
                  )
                  .toList();
              ...
              final providedUnsplashImages = unsplashImages
                  .where((img) => img.contains('images.unsplash.com/photo-'))
                  .toList();
              print(
                '🔍 DEBUG: Used provided Unsplash images: ${providedUnsplashImages.length}/${unsplashImages.length}',
              );
              ...
              final statisticsCount = RegExp(
                r'\d+%|\d+\.\d+%|\d+\/\d+|\d+:\d+',
              ).allMatches(textContent).length;
              final chartElements = RegExp(
                r'chart|graph|progress|bar|pie|line',
              ).allMatches(textContent).length;
              final dataPoints = RegExp(
                r'statistic|data|metric|percentage|rate|growth',
              ).allMatches(textContent).length;
              ...
              if (imageSources.length <= 3 && statisticsCount >= 20) {
                print(
                  '🔍 DEBUG: Excellent - Minimal images with high data density!',
                );
                ...
                print(
                  '🔍 DEBUG: WARNING - Too many images, focus on data density!',
                );
              } else {
                print('🔍 DEBUG: INFO - Current image and data balance');
              }
            }
            // JSON is already parsed into jsonData above
            print('🔍 DEBUG: Finalizing data from jsonData');

            final finalHtml = (jsonData['html']?.toString() ?? '').trim();
            final finalCss = (jsonData['css']?.toString() ?? '').trim();

            if (finalHtml.isEmpty ||
                finalHtml == 'null' ||
                finalHtml.length < 50) {
              print('🔍 DEBUG: API returned insufficient HTML content');
              return null;
            }

            print(
              '🔍 DEBUG: Successfully generated HTML. Length: ${finalHtml.length}',
            );
        ...
        if (e is GenerativeAIException && e.toString().contains('503')) {
          ...
          if (attempt < maxRetries) {
            ...
            final delay = Duration(
              seconds:
                  baseDelay.inSeconds *
                  (attempt * attempt), // Exponential backoff: 2s, 8s, 18s
            );
            ...
            Get.snackbar(
              ...
              duration: Duration(
                seconds: delay.inSeconds.clamp(1, 10),
              ), // Show for max 10 seconds
